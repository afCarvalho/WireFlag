...
public class OldAction {
	...
	/*
	 * public MapPosition getPos() { return perception.getPosition(); }
	 * 
	 * public WorldState getPerception() { return perception; }
	 */

	/*
	 * public double getNUnknownAdjacent(Agent a, Beliefs state) {
	 * 
	 * MapPosition pos = perception.getPosition(); MapPosition aPos =
	 * a.getPos().getMapPosition(); double result = 1; double distance =
	 * Math.abs(pos.getX() - aPos.getX()) + Math.abs(pos.getY() - aPos.getY()) +
	 * 1;
	 * 
	 ...
	 * 
	 * if (p1 != null && p1.isUnknown()) { result += 10 * result / distance; }
	 * else
	 * 
	 * if (p2 != null && p2.isUnknown()) { result += 10 * result / distance; }
	 * else
	 * 
	 * if (p3 != null && p3.isUnknown()) { result += 10 * result / distance; }
	 * else
	 * 
	 * if (p4 != null && p4.isUnknown()) { result += 10 * result / distance; }
	 * 
	 * return result; }
	 */
	...
	public double getValue(Beliefs state, Agent a, MapPosition initialPos,
		 ...
		 * (getNActions())) + ((getNUnknownAdjacent(a, state))); } else if
		 * (option == GO_AWAY) { moveUtility = getMoveUtility(a) * getNActions()
		 * + getNActions(); } else { moveUtility = (((getMoveUtility(a)) + ((1 +
		...
		/*
		 * double stopFactor = getStopUtility(a, state); double illFactor =
		 * getIllUtility(a); double lifeFactor = getLifeUtility(a);
		 * 
		 * if (stopFactor != 0 && stopFactor < a.getFatigue() + 1) { stopUtility
		 * = stopUtility + stopFactor; } else if (illFactor != 0 && illFactor <
		 * 100 - a.getLife()) { stopUtility = stopUtility + illFactor; } else if
		 * (lifeFactor != 0 && lifeFactor < 100 - a.getLife()) { stopUtility =
		 ...
		 * (higherUtility == flagUtility) { action = GET_FLAG; } else if
		 * (higherUtility == endUtility) { action = GET_END; } else
		 */
	...
	/*
	 * public OldAction getAncestor() { return ancestor; }
	 */

	/*
	 * public void setAncestor(OldAction ancestor) { this.ancestor = ancestor; }
	 */
	 ...
	 * 
	 * if (ancestor != null) { ancestor.getActionsList(actions); } }
	 */

	/*
	 * public int getNActions() { if (ancestor == null) { return 1; }
	 * 
	...
	/*
	 * public double getLandUtility(Agent a) { double result =
	 * perception.getLandRating();
	 * 
	 * if (ancestor == null) { return result; } else { return result +
	 * ancestor.getLandUtility(a); }
	 * 
	 * }
	 */
	...
	/*
	 * private double getEndUtility(Beliefs state, Agent a) { if
	 * (state.hasEndPos() && state.getEndPos().isAdjacentPosition(
	 * perception.getPosition(), a.getDirection()) && a.hasFlag()) { return 100;
	 * } else { return 1; } }
	 */
	 ...
	 ***********************/
	/*
	 * public double getStopUtility(Agent a, Beliefs state) { if
	 * (state.shouldStop() && ancestor != null) { return 5 +
	 * ancestor.getStopUtility(a, state); } else if (state.shouldStop() &&
	 * ancestor == null) { return 5; } else { return 0; } }
	 * 
	 * private double getIllUtility(Agent a) { if (a.isIll() && ancestor !=
	 * null) { return 1 + ancestor.getIllUtility(a); } else if (a.isIll() &&
	 * ancestor == null) { return 1; } else { return 0; } }
	 * 
	 * private double getLifeUtility(Agent a) { if (a.hasLowLife() && ancestor
	 * != null) { return 1 + ancestor.getLifeUtility(a); } else if
	 * (a.hasLowLife() && ancestor == null) { return 1; } else { return 0; } }
	 */
}
