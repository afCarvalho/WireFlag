...
public class Deliberative extends Architecture {
	...
	public void makeAction(Agent agent, int delta) {
		...
		for (Action ac : actions) {
			System.out.print(ac.getAction() + "/" + ac.getNActions() + " "
					+ "( " + ac.getPos().getX() + " " + ac.getPos().getY()
					+ " )");
		...
		for (Action ac : actions) {
			...
			|| state.getWorld()[acPos.getX()][acPos.getY()].hasExtremeWeather()) {
		...
		if (a.getAction() == Action.STOP_ACTION) {
			...
			System.out.println("MOVE " + actions.getFirst().getPos().getX()
					...
					+ " stop utility " + a.getStopUtility(agent, state)
					+ " ancestor " + a.getAncestor());
